import torch
from filtro import filtro_boxes
import numpy as np
from config import CLASES_CLASIFICACION
import time
from modelo_ONNX import inferencia_ONNX


def predecir(modelos, imagen, device, indice, id_imagenes_lista, FLAG_ONNX):
    ...
    if tecnica == "DETECCION":
        prediccion_sin_filtro, scores_sin_filtro = predecir_deteccion(
            modelo, imagen, device, FLAG_ONNX)
        prediccion = filtro_boxes(prediccion_sin_filtro, scores_sin_filtro)

    else:
        prediccion = predecir_clasificacion(modelo, imagen, device, FLAG_ONNX)

    return prediccion


def predecir_deteccion(modelo, imagen, device, FLAG_ONNX):
    ...
    if FLAG_ONNX == 0:
        ...
        start_time = time.time()
        pred = inferencia_ONNX(imagen, modelo,  deteccion=1)

        end_time = time.time()
        print(
            f"Tiempo ejecucion deteccion:{(end_time - start_time):.2f} sec")
        boxes = pred[0]
        scores = pred[2]
...
def predecir_clasificacion(modelo, imagen, device, FLAG_ONNX):
    ...
    if FLAG_ONNX == 0:
        ...
        end_time = time.time()
        print(
            f"Tiempo ejecucion clasificacion:{(end_time - start_time):.2f} sec")
    ...
    ## ONNX ##
        ...
        start_time = time.time()
        pred = inferencia_ONNX(imagen, modelo, deteccion=0)
