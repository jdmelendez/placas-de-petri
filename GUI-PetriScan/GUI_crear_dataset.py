...
from GUI_elige_modelos import elige_modelos
import GUI_transformar_dataset


# transformacion=aplica_transformacion):
def crear_dataset(df, path_imagenes,  transformaciones=GUI_transformar_dataset):
    ...
    if path_imagenes[-3:] == 'png':
        ...
        # lista_paths_imgs = [i for i in lista_paths_imgs if "OK" not in i]
        lista_paths_imgs = [i for i in lista_paths_imgs if (
            i[:-4] + "_OK.png" not in lista_paths_imgs) and "OK" not in i]
    ...
    if lista_paths_imgs != []:
        ...
        # Todas las imágenes han sido ya evaluadas"
        return
